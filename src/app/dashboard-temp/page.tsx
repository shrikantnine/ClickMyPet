  ...
  const downloadSelectedImages = async () => {
    ...
    try {
      if (selectedImages.length === 1) {
        ...
        a.href = url
        a.download = `click-my-pet-${selectedImages[0].style}-${Date.now()}.png`
        ...
        const zip = new JSZip()
        const folder = zip.folder('Click-My-Pet-Portraits')
        ...
        const a = document.createElement('a')
        a.href = url
        a.download = `Click-My-Pet-Portraits-${Date.now()}.zip`
        document.body.appendChild(a)
        a.click()
        document.body.removeChild(a)
        URL.revokeObjectURL(url)
